...
files = [f for start_year in range(2014, 2019)
         for f in glob.glob(f"../laliga/{start_year}{start_year+1}.csv")]
...
all_games.to_csv("../results/laliga_focal.csv", index=False)
print(f"Wrote {len(all_games)} rows to results/laliga_focal.csv")
