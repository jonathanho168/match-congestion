...
files = [f for start_year in range(2014, 2019)
         for f in glob.glob(f"data/{start_year}{start_year+1}.csv")]
...
all_games.to_csv("results/focal_results.csv", index=False)
print(f"Wrote {len(all_games)} rows to results/focal_results.csv")
